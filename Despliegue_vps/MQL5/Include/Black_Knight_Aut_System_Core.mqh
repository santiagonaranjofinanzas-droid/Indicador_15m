...
class CAdaptiveControl {
public:
    static double Clamp01(double x) {
        return MathMax(0.0, MathMin(1.0, x));
    }

    static double EWMAClamp(double prev_value, double new_value, double alpha, double min_v, double max_v) {
        double a = Clamp01(alpha);
        double smoothed = a * new_value + (1.0 - a) * prev_value;
        return MathMax(min_v, MathMin(max_v, smoothed));
    }

    static double ShannonEntropyBinary(double p) {
        double q = Clamp01(p);
        q = MathMax(1e-9, MathMin(1.0 - 1e-9, q));
        return -(q * MathLog(q) + (1.0 - q) * MathLog(1.0 - q));
    }
};
