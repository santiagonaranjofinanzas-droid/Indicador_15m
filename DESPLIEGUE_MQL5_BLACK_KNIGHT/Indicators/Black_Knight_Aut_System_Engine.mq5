...
//+------------------------------------------------------------------+
#include <Black_Knight_Aut_System_Core.mqh>
...
#property version   "8.30" // V8.3: OOS Validation + Anti-Snooping Controls
...
// --- FASE 1/2: Institutional Health & Adaptive Control ---
input string G8 = "─── Institutional Stability Control ───";
input int    InpHealthWindow      = 250;   // Ventana para score de salud del alpha
input double InpNuEmaAlpha        = 0.20;  // Suavizado EMA de nu dinámico
input double InpLambdaEmaAlpha    = 0.20;  // Suavizado EMA de lambda dinámica
input double InpNuPriorWeight     = 0.25;  // Shrinkage de nu hacia prior (anti-overfit)
...
input int    InpValidationWindow  = 300;   // Ventana de validación online
input double InpValEmaAlpha       = 0.15;  // Suavizado del score de validación
input double InpValHealthy        = 0.70;  // Umbral estado Healthy
input double InpValWarning        = 0.50;  // Umbral estado Warning
...
// V8.2: Health & calibration diagnostics
double b_health[], b_brier[];
double b_valscore[];
...
double g_health_score  = 1.0;   // Health score del alpha (0..1)
double g_thresh_dyn    = 0.65;  // Threshold dinámico aplicado
double g_strength_gate = 0.30;  // Strength gate dinámico aplicado
double g_validation_score = 1.0;
string g_alpha_state = "HEALTHY";
...
void CalculateKurtosis(int rates_total, const double &src[], double &kurt_buffer[], int period, int start) {
    ...
    for(int i=s; i<rates_total; i++) {
        ...
        // BUG #6 FIX: Fisher bias correction for finite samples
        double n = (double)period;
        double raw_kurt = (m4 / (m2 * m2)) - 3.0;
        if(n > 3.0) {
            double factor = ((n - 1.0) / ((n - 2.0) * (n - 3.0)));
            kurt_buffer[i] = factor * ((n + 1.0) * raw_kurt + 6.0);
        } else {
            kurt_buffer[i] = raw_kurt;
        }
    }
}
...
int OnInit() {
    ...
    // V8.2: Health buffers + one padding slot
    SetIndexBuffer(38, b_health,       INDICATOR_CALCULATIONS); // Health score [0..1]
    SetIndexBuffer(39, b_brier,        INDICATOR_CALCULATIONS); // Calibration proxy
    SetIndexBuffer(40, b_valscore,     INDICATOR_CALCULATIONS); // Validation score [0..1]
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "Black_Knight_Aut_System Quantum Regime Engine V8.3");
...
int OnCalculate(const int rates_total,
    ...
    if(start < min_start) {
        ...
        ArrayInitialize(b_health, 1.0);
        ArrayInitialize(b_brier, 0.25);
        ArrayInitialize(b_valscore, 1.0);
    ...
    // --- ATR Copy (BUG #3 FIX: Direct copy, no index inversion) ---
    int to_copy = calc_limit - start;
    if(to_copy > 0) {
        static double temp_atr[]; ArrayResize(temp_atr, to_copy, 1000);
        int copied = CopyBuffer(g_atr_handle, 0, 0, rates_total, temp_atr);
        if(copied <= 0) return(0);
        for(int i=start; i<calc_limit; i++) b_atr[i] = (i < copied) ? temp_atr[i] : temp_atr[copied-1];
    }
    ...
    // --- Returns (BUG #7 FIX: Aligned to close[i]/close[i-1]) ---
    for(int i=MathMax(1,start); i<calc_limit; i++)
        b_returns[i] = MathLog(close[i] / close[i-1]);
    ...
    for(int i=start; i<calc_limit; i++) {
        ...
        if(i >= InpLongRunW && (i % InpRecalibWindow == 0)) {
            ...
            double nu_raw = g_nu_dynamic;
            double lam_raw = g_lambda_dynamic;

            // ν from excess kurtosis: ν = 6/κ + 4 (MOM estimator for t-Student)
            double kappa = b_kurtosis[i];
            if(kappa > 0.01) nu_raw = MathMax(2.5, MathMin(30.0, 6.0 / kappa + 4.0));
            ...
            lam_raw = MathMax(0.01, MathMin(0.30, (double)jump_count / w));

            // Shrinkage institucional hacia prior para reducir data-snooping
            double nu_w  = CAdaptiveControl::Clamp01(InpNuPriorWeight);
            double lam_w = CAdaptiveControl::Clamp01(InpLambdaPriorWeight);
            double nu_target  = (1.0 - nu_w)  * nu_raw  + nu_w  * ExtHMMNu;
            double lam_target = (1.0 - lam_w) * lam_raw + lam_w * ExtJumpLambda;

            // Límite de paso por actualización (anti-chasing)
            double nu_step  = MathMax(0.1, InpNuMaxStep);
            double lam_step = MathMax(0.001, InpLambdaMaxStep);
            nu_target  = MathMax(g_nu_dynamic - nu_step, MathMin(g_nu_dynamic + nu_step, nu_target));
            lam_target = MathMax(g_lambda_dynamic - lam_step, MathMin(g_lambda_dynamic + lam_step, lam_target));

            // EMA institucional para evitar over-reaction en cambios de régimen
            g_nu_dynamic     = CAdaptiveControl::EWMAClamp(g_nu_dynamic, nu_target,  InpNuEmaAlpha,     2.5, 30.0);
            g_lambda_dynamic = CAdaptiveControl::EWMAClamp(g_lambda_dynamic, lam_target, InpLambdaEmaAlpha, 0.01, 0.30);
        }
        ...
        int lookback = MathMin(i, InpDriftWindow);
        ...
        // =========================================
        // FASE 1: Health score institucional
        // =========================================
        int health_w = MathMin(i, InpHealthWindow);
        if(health_w < 8) health_w = 8;

        b_brier[i] = (i > 0) ? MathPow(b_p1[i-1] - ((b_returns[i] > 0.0) ? 1.0 : 0.0), 2.0) : 0.25;

        double ent_sum = 0.0, brier_sum = 0.0;
        for(int k=0; k<health_w; k++) {
            ent_sum += CAdaptiveControl::ShannonEntropyBinary(b_p1[i-k]);
            brier_sum += b_brier[i-k];
        }

        double ent_mean = ent_sum / health_w;
        double brier_mean = brier_sum / health_w;
        double ent_norm = MathMin(1.0, ent_mean / 0.6931471805599453); // ln(2)
        double brier_norm = MathMin(1.0, brier_mean / 0.25);
        double vol_stress = MathMin(1.0, MathMax(0.0, (vol_ratio - 1.0) / 1.5));

        double health = 0.45 * (1.0 - brier_norm) + 0.35 * (1.0 - ent_norm) + 0.20 * (1.0 - vol_stress);
        health = CAdaptiveControl::Clamp01(health);
        b_health[i] = health;
        g_health_score = health;

        // =========================================
        // FASE 2: Dynamic thresholding by health
        // Combined Regime (Kalman gate replaces HMA)
        // =========================================
        double health_gap = (1.0 - health);
        double thresh_dyn = MathMax(0.55, MathMin(0.90, InpThresh + InpThreshAdaptK * health_gap));
        double strength_gate = MathMax(0.20, MathMin(0.95, InpMinStrength + InpStrengthAdaptK * health_gap));
        g_thresh_dyn = thresh_dyn;
        g_strength_gate = strength_gate;

        bool hmm_bull = hmm_prob > thresh_dyn;
        bool hmm_bear = hmm_prob < (1.0 - thresh_dyn);
        ...
        // --- Entry Signals ---
        b_to_bull[i] = (regime==1 && b_regime[MathMax(i-1,0)]!=1 && b_strength[i]>strength_gate)
                       ? low[i] - 10*_Point : EMPTY_VALUE;
        b_to_bear[i] = (regime==-1 && b_regime[MathMax(i-1,0)]!=-1 && b_strength[i]>strength_gate)
        ...
        // =========================================
        // FASE 3: Online validation by rolling block
        // =========================================
        int val_w = MathMin(i - 1, InpValidationWindow);
        ...
        for(int k=0; k<val_w; k++) {
            ...
            // Validación estricta OOS: señal en t-1 evaluada con retorno realizado en t
            int reg_k = (int)b_regime[pred_idx];
            if(reg_k == 0) continue;

            double r_k = b_returns[idx];
            bool correct = ((reg_k == 1 && r_k > 0.0) || (reg_k == -1 && r_k < 0.0));
            acc_sum += correct ? 1.0 : 0.0;
            acc_n++;

            double signed_ret = (reg_k == 1) ? r_k : -r_k;
            edge_sum += signed_ret;
            edge_sq += signed_ret * signed_ret;
        }

        double acc = (acc_n > 0) ? (acc_sum / acc_n) : 0.5;
        double edge_score = 0.5;
        if(acc_n > 8) {
            double e_mean = edge_sum / acc_n;
            double e_var = MathMax(edge_sq / acc_n - e_mean * e_mean, 1e-12);
            double e_t = (e_mean / MathSqrt(e_var)) * MathSqrt((double)acc_n);
            edge_score = CStatistics::LogisticClamped(e_t);
        }

        double val_raw = CAdaptiveControl::Clamp01(0.50 * health + 0.35 * acc + 0.15 * edge_score);
        double val_prev = (i > 0) ? b_valscore[i-1] : val_raw;
        double val_score = CAdaptiveControl::EWMAClamp(val_prev, val_raw, InpValEmaAlpha, 0.0, 1.0);
        b_valscore[i] = val_score;
        g_validation_score = val_score;

        // =========================================
        // FASE 4: Production state machine
        // =========================================
        if(val_score >= InpValHealthy && health >= 0.70) {
            g_alpha_state = "HEALTHY";
        } else if(val_score >= InpValWarning) {
            g_alpha_state = "WARNING";
        } else {
            g_alpha_state = "DEFENSIVE";
        }
    }
...
void DrawDashboard() {
    ...
    string reg_str = (g_hmm_prob > g_thresh_dyn) ? "▲ BULL" : (g_hmm_prob < (1.0-g_thresh_dyn)) ? "▼ BEAR" : "● NEUT";
    color  reg_col = (g_hmm_prob > g_thresh_dyn) ? C_BULL : (g_hmm_prob < (1.0-g_thresh_dyn)) ? C_BEAR : C_NEUT;
    ...
    CreateLabel("HMM_Health",   "Health:    " + DoubleToString(g_health_score*100.0,1) + "%", x_off, y_off+138, corner, 9, g_health_score<0.5?clrTomato:clrSilver);
    CreateLabel("HMM_Dyn",      "Dyn Gate:  P>" + DoubleToString(g_thresh_dyn,2) + " | S>" + DoubleToString(g_strength_gate,2), x_off, y_off+154, corner, 9, clrDimGray);
    CreateLabel("HMM_Val",      "ValScore:  " + DoubleToString(g_validation_score*100.0,1) + "%", x_off, y_off+170, corner, 9, g_validation_score<InpValWarning?clrTomato:clrSilver);
    CreateLabel("HMM_State",    "State:     " + g_alpha_state, x_off, y_off+186, corner, 9, g_alpha_state=="HEALTHY"?clrSpringGreen:(g_alpha_state=="WARNING"?clrGold:clrTomato));
}
