...
#property strict
#property tester_file "..\\Files\\HMM_Params_1M.csv"
...
int OnInit() {
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "Regime HMM EWMA (1M)");
    
    // V7: CSV parsing with Numerical Stability support (15 columns)
    int handle = FileOpen("HMM_Params_1M.csv", FILE_READ | FILE_CSV | FILE_ANSI, ',');
    if(handle == INVALID_HANDLE) {
        PrintFormat("CRITICAL ERROR: Could NOT open HMM_Params_1M.csv in %s mode. Error %d", 
    ...
    if(handle != INVALID_HANDLE) {
        ...
        if(!FileIsEnding(handle)) {
            ...
            if(StringLen(vals[0]) > 0 && StringLen(vals[1]) > 0) {
                ...
                PrintFormat("V7 Loaded 1M: PBull=%.3f λ=%.3f ν=%.2f Weights=[%.2f,%.2f,%.2f,%.2f] Mu=[%.2f,%.2f,%.2f]",
                            ExtPBull, ExtJumpLambda, ExtHMMNu, ExtWConf, ExtWVol, ExtWSlope, ExtWInter, ExtMuConf, ExtMuVol, ExtMuSlope);
            }
        }
        FileClose(handle);
    } else {
        Print("Could not load HMM_Params_1M.csv, using default fallbacks. Error: ", GetLastError());
    }
...
void DrawDashboard() {
    ...
    CreateLabel("HMM_Title",    "── HMM EWMA 1M ──",                                     x_off, y_off,      corner, 9,  clrDimGray);
