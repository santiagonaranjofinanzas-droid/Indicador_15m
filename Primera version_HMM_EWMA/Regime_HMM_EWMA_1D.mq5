...
#property strict
#property tester_file "..\\Files\\HMM_Params_1D.csv"
...
// Plot 2: HMA Halo (Color Line - glow effect)
...
double ExtPBull      = 0.999;   // P(Bull|Bull)
double ExtPBear      = 0.928;   // P(Bear|Bear)
double ExtSlopeT     = 0.003;   // Slope threshold (V6: Now dynamic for 1D too)
double ExtJumpLambda = 0.05;    // Jump intensity (λ)
double ExtHMMNu      = 5.11;    // t-Student DoF (ν)
...
input double InpSlopeT      = 0.003;  // Slope threshold
input bool   InpHMAReq      = true;   // Require HMA confirm
input bool   InpHMAShow     = true;   // Show HMA

input string G3 = "─── EWMA Volatility ───";
input double InpLambda      = 0.957;  // Lambda (Decay)
input int    InpLongRunW    = 120;    // Long-run vol window

input string G4 = "─── Execution & Display ───";
input double      InpMinStrength      = 0.3;          // Minimum Strength for Trade Confirmation (0=disabled)
input bool   InpShowBg      = true;   // Background tint
...
double LogTStudent(double x, double mu, double sig, double nu) {
    if(sig <= 0) sig = 1e-10;
    double z = (x - mu) / sig;
    // V7 Parity: Include normalization constants
...
int OnInit() {
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "Regime HMM EWMA (1D)");

    // V7: CSV parsing with Numerical Stability support (15 columns)
    int handle = FileOpen("HMM_Params_1D.csv", FILE_READ | FILE_CSV | FILE_ANSI, ',');
    if(handle == INVALID_HANDLE) {
        PrintFormat("CRITICAL ERROR: Could NOT open HMM_Params_1D.csv in %s mode. Error %d", 
    ...
    if(handle != INVALID_HANDLE) {
        ...
        if(!FileIsEnding(handle)) {
            ...
            if(StringLen(vals[0]) > 0 && StringLen(vals[1]) > 0) {
                ...
                PrintFormat("V7 Loaded 1D: PBull=%.3f λ=%.3f ν=%.2f Weights=[%.2f,%.2f,%.2f,%.2f] Mu=[%.2f,%.2f,%.2f]",
                            ExtPBull, ExtJumpLambda, ExtHMMNu, ExtWConf, ExtWVol, ExtWSlope, ExtWInter, ExtMuConf, ExtMuVol, ExtMuSlope);
            }
        }
        FileClose(handle);
    } else {
        Print("Could not load HMM_Params_1D.csv, using default fallbacks. Error: ", GetLastError());
    }
...
int OnCalculate(const int rates_total,
    ...
    // CopyBuffer returns data in reverse-chronological order (newest first at index 0).
    // Our b_atr[] uses chronological order (oldest first). We reverse the mapping.
    ...
    // 3. Pre-calculate HMA over shifted_close array
    ...
    for(int i=start; i<calc_limit; i++) {
        ...
        // mu_bull = EMA of max(ret, 0): average magnitude of positive returns
        // mu_bear = EMA of |min(ret, 0)|: average magnitude of negative returns
        ...
        double prev_p1 = b_p1[i-1];
        double p1_pred = ExtPBull * prev_p1 + (1.0 - ExtPBear) * (1.0 - prev_p1);
        double p0_pred = 1.0 - p1_pred;

        // V4 Fix #4: Asymmetric likelihood — bull uses +mu_bull, bear uses -mu_bear
        ...
        // Multiplier = sqrt(max(excess_kurtosis + 3, 3)), clamped to [2.0, 10.0]
        ...
        double hma_thresh_abs = b_atr[MathMax(i-1, 0)] * InpSlopeT;
        ...
        double ewma_sigma = MathMax(MathSqrt(MathMax(b_sigma2_ewma[i], 0.0)), 1e-12);
        double lr_sigma = MathMax(b_lr_sigs[i], 1e-12);
        double vol_ratio = ewma_sigma / lr_sigma;
        
        if(!MathIsValidNumber(vol_ratio)) vol_ratio = 1.0;
        vol_ratio = MathMin(vol_ratio, 50.0); // Cap extreme outliers for 1D stability

        // --- V7 ML Strength Score (Ridge + Standardization) ---
        double conf = MathAbs(hmm_prob - 0.5) * 2.0;
        double hma_slope_mag = MathAbs(hma_slope) / MathMax(hma_thresh_abs / (ExtSlopeT > 0 ? ExtSlopeT : 0.003), 1e-10);
        ...
        // --- Entry Signals (V4 Fix #10: optional strength filter) ---
        ...
        if(InpShowRegArr && i > start) {
            int prev_regime = (int)b_regime[MathMax(i-1,0)];
            bool regime_changed = (regime != prev_regime);
            b_regime_bull[i] = (regime_changed && regime == 1)  ? low[i]  - 25 * _Point : EMPTY_VALUE;
            b_regime_bear[i] = (regime_changed && regime == -1) ? high[i] + 25 * _Point : EMPTY_VALUE;
        } else {
...
void DrawDashboard() {
    ...
    CreateLabel("HMM_Title",    "── HMM EWMA 1D ──",                                     x_off, y_off,      corner, 9,  clrDimGray);
