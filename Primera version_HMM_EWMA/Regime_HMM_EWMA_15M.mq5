...
double ExtMuAccel    = 1.0, ExtStdAccel = 2.0;
...
double b_hma_raw_slope[], b_hma_raw_accel[], b_hma_val[];
...
int OnInit() {
    ...
    SetIndexBuffer(35, b_hma_raw_accel, INDICATOR_CALCULATIONS);
    SetIndexBuffer(36, b_temp_d,       INDICATOR_CALCULATIONS); // Re-assigning or adding new
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "Regime HMM EWMA (15M) [V7.1 Accel]");
    
    // V7.1: CSV parsing with Acceleration Support (18 columns)
    int handle = FileOpen("HMM_Params_15M.csv", FILE_READ | FILE_CSV | FILE_ANSI, ',');
    if(handle == INVALID_HANDLE) {
        PrintFormat("CRITICAL ERROR: Could NOT open HMM_Params_15M.csv. Error %d", GetLastError());
        return(INIT_SUCCEEDED); // Return succeeded to allow manual correction but show error
    }
    
    if(handle != INVALID_HANDLE) {
        // Read header row (18 columns)
        string cols[18]; for(int i=0; i<18; i++) cols[i] = FileReadString(handle);
        
        bool has_stability = (cols[10] == "MuConf" && cols[17] == "StdAccel");
        
        if(!FileIsEnding(handle)) {
            string vals[18]; for(int i=0; i<18; i++) vals[i] = FileReadString(handle);
            if(StringLen(vals[0]) > 0 && StringLen(vals[1]) > 0) {
                ...
                ExtWSlope = StringToDouble(vals[7]); ExtWAccel = StringToDouble(vals[8]);
                ExtWInter = StringToDouble(vals[9]);
                
                if(has_stability) {
                    ExtMuConf  = StringToDouble(vals[10]);  ExtMuVol  = StringToDouble(vals[11]); 
                    ExtMuSlope = StringToDouble(vals[12]);  ExtMuAccel = StringToDouble(vals[13]);
                    ExtStdConf = StringToDouble(vals[14]);  ExtStdVol = StringToDouble(vals[15]); 
                    ExtStdSlope = StringToDouble(vals[16]); ExtStdAccel = StringToDouble(vals[17]);
                    
                    if(ExtStdConf < 1e-10) ExtStdConf = 1.0; if(ExtStdVol < 1e-10) ExtStdVol = 1.0; 
                    if(ExtStdSlope < 1e-10) ExtStdSlope = 1.0; if(ExtStdAccel < 1e-10) ExtStdAccel = 1.0;
                }
                PrintFormat("V7.1 Loaded 15M: PBull=%.3f λ=%.3f AccelW=%.2f Weights=[%.2f,%.2f,%.2f,%.2f]",
                            ExtPBull, ExtJumpLambda, ExtWAccel, ExtWConf, ExtWVol, ExtWSlope, ExtWInter);
            }
        }
        FileClose(handle);
    }
...
int OnCalculate(const int rates_total,
    ...
    g_strength = 0.0; g_hmm_prob = 0.5; g_vol_ratio = 1.0; g_confidence = 0.0;
    ...
    if(start < min_start) {
        ...
        ArrayInitialize(b_strength, 0.0);
    }
    ...
    for(int i=start; i<calc_limit; i++) {
        ...
        // Acceleration
        double hma_slope_prev = (i >= InpSlopeN) ? ( (b_hma_val[i-InpSlopeN] - b_hma_val[MathMax(i-2*InpSlopeN, 0)]) / (double)InpSlopeN ) : 0;
        b_hma_raw_accel[i] = (b_hma_raw_slope[i] - hma_slope_prev) / (double)InpSlopeN;
        
        double accel_sum = 0;
        for(int j=0; j<InpSlopeS; j++) accel_sum += b_hma_raw_accel[MathMax(i-j, 0)];
        double hma_accel = accel_sum / InpSlopeS;
        ...
        // --- V7.1 ML Strength Score (Extended Acceleration) ---
        ...
        double hma_accel_mag = MathAbs(hma_accel) / MathMax(hma_thresh_abs / (ExtSlopeT > 0 ? ExtSlopeT : 0.0273), 1e-10);
        ...
        double s_accel = (hma_accel_mag - ExtMuAccel) / ExtStdAccel;
        ...
        double st_val = 1.0 / (1.0 + MathExp(-MathMax(MathMin(z, 20), -20)));
        b_strength[i] = MathIsValidNumber(st_val) ? st_val : 0.0;
...
void DrawDashboard() {
    ...
    CreateLabel("HMM_Title",    "── HMM EWMA 15M ──",                                    x_off, y_off,      corner, 9,  clrDimGray);
